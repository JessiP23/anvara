    ...
    return (
        <form action={formAction} className="space-y-4 text-black">
            ...
            <FormField id='name' label='name' required error={state.fieldErrors?.name}>
                <input type='text' id='name' name='name' defaultValue={campaign?.name} className='mt-1 w-full rounded border p-2' required />
            </FormField>
            
            <FormField id='description' label='description' required error={state.fieldErrors?.description}>
                <textarea id='description' name='description' defaultValue={campaign?.description} rows={3} className='mt-1 w-full rounded border p-2' required />
            </FormField>

            <FormField id='budget' label='Budget ($)' required error={state.fieldErrors?.budget}>
                <input type='number' id='budget' name='budget' defaultValue={campaign?.budget} min='1' className='mt-1 w-full rounded border p-2' required />
            </FormField>

            <div className='grid grid-cols-2 gap-4'>
                <FormField id='startDate' label='Start Date' required error={state.fieldErrors?.startDate}>
                    <input type='date' id='startDate' name='startDate' defaultValue={formatDate(campaign?.startDate)} className='mt-1 w-full rounded border p-2' required />
                </FormField>
                <FormField id='endDate' label='End Date' required error={state.fieldErrors?.endDate}>
                    <input type='date' id='endDate' name='endDate' defaultValue={campaign?.endDate} className='mt-1 w-full rounded border p-2' required />
                </FormField>
            </div>
